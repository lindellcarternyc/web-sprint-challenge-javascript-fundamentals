...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// Nested function can access the internal variable because of a concept called closures. 
// The nested function "closes over" values in its parent's scope.
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(animalData) {
    const displayNames = []
    animalData.forEach(animal => displayNames.push(`name: ${animal.animal_name}, scientific: ${animal.scientific_name}`))
    return displayNames
  }
  ...
  */
  ...
  function lowerCaseNames(animalData){
    return animalData.map(animal => `${animal.animal_name.toLowerCase()}`)
  }
  ...
  */
  ...
  function USApop(animalData){
    return animalData.reduce((total, animal) => total + animal.population, 0)
  }
...
const cube1 = new CubeMaker(12)
console.log('cube volume', cube1.volume())
console.log('cube surface area', cube1.surfaceArea())
